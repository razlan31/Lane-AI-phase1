-- Phase 1: Complete Database Foundation (Fixed)

-- First clear existing data and add proper constraints
DELETE FROM tool_block_links;
DELETE FROM blocks WHERE venture_id IS NULL;
DELETE FROM tools;

-- Add unique constraint for blocks
ALTER TABLE blocks ADD CONSTRAINT unique_block_name_category UNIQUE (category, name);

-- Insert comprehensive blocks taxonomy (180+ blocks across 9 categories)
...
('business_model', 'Market Sizing', 'Calculate total addressable market', 'planned', ARRAY['market', 'sizing']),
...
('team', 'Diversity and Inclusion', 'Promote diversity and inclusion', 'planned', ARRAY['diversity', 'inclusion']),
...
('swot_analyzer', 'Organizational Design'),
